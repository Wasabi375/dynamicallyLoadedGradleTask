...
fun readArgs(args: Array<String>): IncrementalInput {
    ...
    check(args.size >= 6) { "Expected at least 6 arguments" }

    val isIncremental = args[0].toBoolean()

    val totalCount = args[1].toInt()

    val (modified, addStart) = getNextFiles(args, 2)
    ...
    return IncrementalInput(isIncremental, fileData.toList())
}
